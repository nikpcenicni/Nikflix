...
8. Never commit or push on your own initiative — stage and describe changes, leave `git commit`/`git push` to the user unless explicitly asked to in that turn. If you do commit, never add Claude as a contributor — no `Co-Authored-By: Claude ...` trailer or other AI-attribution line, in the commit message or a PR description.
