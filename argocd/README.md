...
| `homepage` | Helm chart `app-template` from the bjw-s chart repository, values in `dev/values/homepage-values.yaml` | `homepage` | [Homepage](https://gethomepage.dev) - admin overview dashboard for every app in this cluster, grouped into sections (Media Requests, Arr Stack, Downloads, Monitoring, Platform) with live per-tile status widgets - see [Homepage](#homepage). Serves `dash.lab.pcenicni.dev`. Deliberately *not* the bare `lab.pcenicni.dev` apex: that single-label name is shadowed by the existing Pangolin `*.pcenicni.dev` wildcard CNAME in the public Cloudflare zone, so unlike every other `*.lab.pcenicni.dev` host (which cleanly NXDOMAINs if a client's DNS ever slips off the pi-hole override), the apex would need a permanent placeholder record to avoid falling through to Pangolin - and that placeholder itself is publicly resolvable/proxied, so any client that queries public DNS for it even briefly gets routed to a live Cloudflare edge with nothing behind it (a 522), instead of just failing to resolve like its siblings. `dash.` avoids that whole failure class the same way the rest of the stack does. Behind the same domain-level `media-authentik-forward-auth` Middleware the media stack uses (cross-namespace reference), since it surfaces status/tokens for every other admin tool. Depends on `authentik-outpost` having synced first, same as the media stack. |
...
| `metrics-server` | Helm chart `metrics-server` from the official metrics-server chart repository, values in `dev/values/metrics-server-values.yaml` | `kube-system` | Kubernetes Metrics API (`kubectl top nodes`/`kubectl top pods`). Runs with `--kubelet-insecure-tls` - confirmed live as necessary: Talos's kubelet serving certs are valid but only SAN the node's hostname, not its IP, so scraping via `InternalIP` (this chart's default preferred address type) fails certificate verification otherwise. This only disables metrics-server's outbound verification of each kubelet's own cert; the Metrics API it serves to callers (`kubectl top`, Ballast, HPA) still goes over its own generated serving certificate. `ballast` depends on this for its default `kubernetesMetrics` `MetricsSource`. |
| `profilarr` | Helm chart `app-template` from the bjw-s chart repository, values in `dev/values/profilarr-values.yaml` | `media` | Configuration management for Sonarr/Radarr custom formats and quality profiles ([Dictionarry-Hub/profilarr](https://github.com/Dictionarry-Hub/profilarr)). Two containers in the pod: `main` (the web UI/API, port 6868) and `parser` (custom format/quality profile testing, reached over `localhost` rather than a Docker Compose service name). `AUTH=off` - sits behind the shared forward-auth middleware like the rest of the admin-tool apps, not Profilarr's own native OIDC support. See [Media stack](#media-stack). |
...
| `argocd-config` | Raw manifests in `dev/manifests/argocd-config/` | `argocd` | Patches the `argocd-cm`, `argocd-rbac-cm`, and `argocd-cmd-params-cm` ConfigMaps that ArgoCD's own install created. Wires up OIDC login against authentik, maps the `ArgoCD Admins`/`ArgoCD Viewers` authentik groups to ArgoCD's built-in `admin`/`readonly` roles, and keeps `argocd-server` running in `--insecure` mode since Traefik terminates TLS. Also owns three `ServiceMonitor` resources (`servicemonitors.yaml`) for ArgoCD's `argocd-metrics`, `argocd-server-metrics`, and `argocd-notifications-controller-metrics` Services - the vendored `argocd` install creates those Services but no ServiceMonitor, so kube-prometheus-stack's Prometheus never scraped `argocd_*` metrics without these, leaving the community "ArgoCD" Grafana dashboard (ID 14584) empty. |
| `authentik-outpost` | Raw manifests in `dev/manifests/authentik-outpost/` | `authentik` | Deployment/Service for the media stack's authentik proxy outpost - see [SSO: authentik forward-auth](#sso-authentik-forward-auth-domain-level). Not authentik-managed (no service-connection configured), so tracked like any other by-hand Deployment. Also carries its own ServiceMonitor for its `:9300` metrics port - most of the Grafana authentik dashboard's panels are `authentik_outpost_*` metrics that only this outpost exports, not the main server/worker (see `authentik`'s row above). Depends on the `authentik` Application's blueprint and the `authentik-outpost-media-token` SopsSecret. |
...
`seerr`, `sonarr`, `radarr`, `prowlarr`, `qbittorrent`, `sabnzbd`,
`bazarr`, and `profilarr` (all in the `media` namespace) are the Talos-side
half of [docs/architecture/media-stack.md](../docs/architecture/media-stack.md).
Jellyfin itself runs on a Mac Mini outside the cluster and outside this
GitOps tree entirely - see that doc for why. All eight Applications use the
...
`sonarr`, `radarr`, `prowlarr`, `bazarr`, `qbittorrent`, `sabnzbd`, and
`profilarr` sit behind authentik SSO - `seerr` deliberately doesn't, since it's the
...
admin overview dashboard for this cluster - `dash.lab.pcenicni.dev` (not
the bare `lab.pcenicni.dev` apex - see the Applications table entry above
for why: it collides with the existing Pangolin `*.pcenicni.dev` wildcard
in the public Cloudflare zone). Same
