...
variable "vm_storage" {
  description = "Proxmox storage ID backing VM boot disks, EFI disks, and the Longhorn data disk. Assumes a Proxmox-managed storage pool, not raw disk passthrough - see the README for the passthrough alternative, which docs/architecture/talos.md leaves as an open question."
  type        = string
  default     = "local-lvm"
}
...
variable "enable_storage_nic" {
  description = "Whether to attach the second NIC (VLAN 80, Longhorn storage) to each VM. Requires the vmbr2 bridge to already exist on the target node (see README prerequisites) - set false to stand up a VM before that bridge is rolled out, e.g. for a smoke test."
  type        = bool
  default     = true
}

variable "cpu_type" {
  description = "QEMU CPU type for the Talos VMs. x86-64-v2-AES is a portable baseline across the noble nodes' hardware, at some cost vs. `host`; revisit once actual CPU models are confirmed identical across all six nodes."
  type        = string
  default     = "x86-64-v2-AES"
}

variable "enable_worker_vms" {
  description = <<-EOT
    Whether to create the three worker VMs (talos-worker-krypton/xenon/radon).
    Their Proxmox hosts are still "planned" per docs/architecture/proxmox.md - leave this false
    until krypton/xenon/radon exist and have joined the noble cluster (phased rollout step 2 in
    docs/architecture/talos.md). Flipping this on before then will fail with "node does not exist".
  EOT
  type        = bool
  default     = false
}
